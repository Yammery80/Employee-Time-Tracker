  ...
  try {
    ...
    const rows = await Promise.all(allDocs.map(async r => {
      ...
      const ajustadoTag = r.ajustado && !r.esAnticipado
        ...
        : "";
      // Only allow edit if not yet approved (or always, admin override)
      const editBtn = `<button class="btn btn-sm" title="Ajustar horas" onclick="openEditModal('${r.id}','${r.fecha}','${r.entrada||""}','${r.salida||""}')" style="padding:4px 8px;border-color:var(--jungle-mid);color:var(--jungle-pale)"><i class="ti ti-pencil"></i></button>`;
      return `<tr>
        <td>${name}${ajustadoTag}</td>
...
window.saveEditModal = async function() {
  ...
  showLoading(mode === "edit" ? "Guardando ajuste..." : "Creando registro anticipado...");
  try {
    if (mode === "edit") {
      ...
      closeEditModal();
      // Recargar la vista que esté activa
      ...
      // ── ADVANCE: buscar el registro que la empleada YA tiene ese día
      // y solo adelantarle la hora de salida. No crear nada nuevo.
      ...
      const recDoc = existing[0];
      // Solo actualizar la salida — la entrada y el formulario de cierre
      // quedan intactos, y el status sigue pendiente hasta que el admin apruebe
      await updateDoc(doc(db,"records", recDoc.id), {
        salida,
